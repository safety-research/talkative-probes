...
def load_full_tuned_lens(
    ...
    try:
        ...
        if not checkpoint_path_or_name:
            ...
            if is_main_process and log:
                log.info(f"Loading TunedLens from default HuggingFace location for model")
...
def initialize_consistency_lens_projection(
    ...
    if is_main_process:
        log.info(f"Attempting 'tuned_lens_composition' initialization for {component_name}")
    ...
    if not tl_checkpoint_path:
        if is_main_process:
            log.info(f"No checkpoint specified - will auto-load pre-trained TunedLens for '{tl_model_name}' from HuggingFace")
    ...
    if tuned_lens is None:
        if is_main_process:
            log.error(f"Failed to load TunedLens for {component_name}. Skipping initialization.")
        return

    # Get the number of layers in the base model
    num_model_layers = tuned_lens.config.num_hidden_layers
    if is_main_process:
        log.info(f"TunedLens has {len(tuned_lens.layer_translators)} translators for {num_model_layers} layer model")
    ...
    if src_layer_idx is None:
        if is_main_process:
            log.error(f"'layer_l' not found in config. Cannot determine source layer for tuned lens initialization.")
        return

    # Validate source layer
    if src_layer_idx >= num_model_layers:
        if is_main_process:
            log.error(f"Source layer {src_layer_idx} is out of bounds for model with {num_model_layers} layers")
        return

    is_per_layer = getattr(model_component.config, 'per_layer_projections', False) if hasattr(model_component, 'config') else False

    if not is_per_layer:
        # Single projection layer mode
        # For single projection, we need to specify which decoder layer we're targeting
        tgt_layer_idx = component_config.get('projection_init_tl_comp_tgt_layer', 0)  # Default to layer 0
        
        if is_main_process:
            log.info(f"Single projection mode: mapping from extraction layer {src_layer_idx} to decoder layer {tgt_layer_idx}")

        # Validate target layer
        if tgt_layer_idx >= num_model_layers:
            if is_main_process:
                log.error(f"Target layer {tgt_layer_idx} is out of bounds for model with {num_model_layers} layers")
            return
        ...
        if any(x is None for x in [W_src, b_src, W_tgt, b_tgt]):
            if is_main_process:
                log.error(f"Failed to extract translator parameters for layers {src_layer_idx} and {tgt_layer_idx}")
            return
        ...
        try:
            ...
            if is_main_process:
                log.error(f"Error computing composed projection: {e}", exc_info=True)
            return

        if not hasattr(model_component, 'proj') or model_component.proj is None:
            if is_main_process:
                log.error(f"{component_name} does not have 'proj' attribute")
            return
        ...
        if is_main_process:
            log.info(f"Initialized {component_name}.proj using tuned_lens_composition "
                     f"(src_layer={src_layer_idx}, tgt_layer={tgt_layer_idx})")
        ...
        if not hasattr(model_component, 'proj_weight') or not hasattr(model_component, 'proj_bias'):
            if is_main_process:
                log.error(f"{component_name} with per_layer_projections=True does not have 'proj_weight' and 'proj_bias' attributes")
            return
        ...
        if is_main_process:
            log.info(f"Per-layer projection mode: mapping from extraction layer {src_layer_idx} to each decoder layer")
        ...
        for decoder_layer_idx in range(n_layers):
            ...
            if tgt_layer_idx >= num_model_layers:
                if is_main_process:
                    log.warning(f"Target layer {tgt_layer_idx} is out of bounds for model with {num_model_layers} layers. Skipping decoder layer {decoder_layer_idx}")
                continue
            ...
            if any(x is None for x in [W_src, b_src, W_tgt, b_tgt]):
                if is_main_process:
                    log.warning(f"Failed to extract translator parameters for decoder layer {decoder_layer_idx} "
            ...
            with torch.no_grad():
                model_component.proj_weight[decoder_layer_idx].data.copy_(W_P.to(dtype=model_component.proj_weight.dtype))
                if use_composed_bias:
                    model_component.proj_bias[decoder_layer_idx].data.copy_(b_P.to(dtype=model_component.proj_bias.dtype))
                else:
                    torch.nn.init.zeros_(model_component.proj_bias[decoder_layer_idx].data)

            # # Log condition number of this projection layer
            # try:
            #     cond_number = torch.linalg.cond(W_P.float()).item()
            #     if is_main_process:
            #         log.info(f"Decoder layer {decoder_layer_idx}: projection condition number = {cond_number:.2e}")
            # except Exception as e:
            #     if is_main_process:
            #         log.warning(f"Could not compute condition number for decoder layer {decoder_layer_idx}: {e}")

            initialized_count += 1

        if is_main_process:
            log.info(f"Initialized {initialized_count}/{n_layers} per-layer projections for {component_name} "
