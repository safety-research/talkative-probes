#!/usr/bin/env python3
"""
Final comprehensive validation that generate_soft, generate_soft_kv_cached, and generate_soft_kv_cached_nondiff
produce identical outputs across different models and configurations.

This version includes proper RNG state management to avoid spurious mismatches.
"""
...
def compare_three_methods(
    ...
    """Compare outputs from all three generation methods with proper RNG management."""
    ...
    try:
        ...
        for max_length in test_lengths:
            for seed_idx, test_seed in enumerate(test_seeds):
                ...
                set_all_seeds(test_seed + 1000)  # Different seed for activation
...
def main():
    """Run comprehensive validation tests with proper RNG management."""
    ...
    if all_tests_passed:
        ...
        print("  1. generate_soft and generate_soft_kv_cached are functionally equivalent")
        print("  2. generate_soft_kv_cached_nondiff also produces identical results")
        print("  3. The KV caching optimization preserves exact generation behavior")
        print("  4. Multi-layer patching works correctly with all generation methods")
    else:
