...
CHECKPOINT_PATH = "/workspace/kitf/talkative-probes/consistency-lens/outputs/checkpoints/gemma3_4b_frozen_nopostfix_GG32POS_gemma-3-4b-pt_L22_e22_frozen_lr7e-4_t8_2ep_resume_0714_111655_frozenenc_add_patch_lowEChighElrscale_beta_OTF_dist2_slurm6046"
CHECKPOINT_PATH_CHATTUNED = "/workspace/kitf/talkative-probes/consistency-lens/outputs/checkpoints/gemma3_4b_frozen_nopostfix_GG32POS_gemma-3-4b-pt_L22_e22_frozen_lr7e-4_t8_2ep_resume_0714_111655_frozenenc_add_patch_lowEChighElrscale_beta_OTF_dist2_slurm6046"
# CHECKPOINT_PATH = "/workspace/kitf/talkative-probes/consistency-lens/outputs/checkpoints/gemma3_27b_frozen_nopostfix_GG32POS_gemma-3-27b-pt_L45_e45_frozen_lr3e-4_t8_2ep_resume_0714_165800_frozenenc_add_patch5_suffix1p0enc_OTF_dist8_slurm6052"
# CHECKPOINT_PATH_CHATTUNED = "/workspace/kitf/talkative-probes/consistency-lens/outputs/checkpoints/gemma3_27b_frozen_nopostfix_GG32POS_gemma-3-27b-pt_L45_e45_frozen_lr3e-4_t8_2ep_resume_0714_165800_frozenenc_add_patch5_suffix1p0enc_OTF_dist8_slurm6052"
...
# CHECKPOINT_PATH_CHATTUNED_BASECHAT = "/workspace/kitf/talkative-probes/consistency-lens/outputs/checkpoints/gemma2_CHAT_9b_frozen_nopostfix_SUS_gemma-2-9b-it_L30_e30_frozen_lr3e-4_t8_2ep_resume_0712_010012_frozenenc_add_patchsmalllrEclip0p01wide_entboost_ITBASE_more_OTF_dist4_slurm4631/checkpoint_step198000_epoch2_.pt"
...
# %%
df = analyzerchat.analyze_all_tokens(textinputwrong, batch_size=8, no_eval=False, move_devices=False, return_structured=True, temperature=0.1, logit_lens_analysis=False, no_kl=True, calculate_token_salience=True,optimize_explanations_config={'just_do_k_rollouts': 8, 'use_batched': True, 'temperature': 1.0, 'batch_size_for_rollouts': 1})
...
input_ids = analyzerchattokenizer.apply_chat_template(chess_sand_messages, return_tensors="pt", return_dict=True).to("cuda")
textinputchess=(analyzerchattokenizer.decode(input_ids['input_ids'][0][1:]))
# %%
dfche= analyzerchat.analyze_all_tokens(textinputchess, batch_size=64, no_eval=True, move_devices=True, return_structured=True, temperature=0.1, logit_lens_analysis=False, no_kl=True, calculate_token_salience=True,optimize_explanations_config={'just_do_k_rollouts': 64, 'use_batched': True, 'temperature': 1.0, 'batch_size_for_rollouts': 8})
...
for i, cont in enumerate(chessconts):
    ...
    print()
# %%
dfchess1shot = analyzerchat.analyze_all_tokens(chessconts[0], batch_size=64, no_eval=False, move_devices=False, return_structured=True, temperature=0.1, logit_lens_analysis=True, no_kl=True, calculate_token_salience=True,optimize_explanations_config={'just_do_k_rollouts': 64, 'use_batched': True, 'temperature': 1.0, 'batch_size_for_rollouts': 8})
# %%
...
if strong_ordering_ids:
    ...
    for _, row in strong_ordering_df.iterrows():
        print(f"  - {row['question_id']} (Neutral effect: {row['position_effect_neutral']:.2f}, Harmful effect: {row['position_effect_harmful']:.2f}): question: {queries_df[queries_df['question_id'] == row['question_id']]['neutral_prompt'].iloc[0]}")
