...
if [ ! -d "$PRETOKENIZED_PATH" ] || [ "$FORCE_REDUMP" == "true" ]; then
    ...
    python scripts/pretokenize_dataset.py \
        --config-path=conf --config-name=simplestories_frozen
else
...
TRAIN_CMD="python scripts/01_train.py --config-name=simplestories_frozen +wandb.name=\"ss_frozen_t10_${SLURM_JOB_ID}\" run_name=\"ss_frozen_t10_${SLURM_JOB_ID}\""
