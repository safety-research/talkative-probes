...
def main() -> None:  # noqa: D401
    ...
    for idx in range(args.num_samples):
        ...
        toks_A = _pad(toks_A, args.seq_len, tokenizer.pad_token_id)
        toks_Ap = _pad(toks_Ap, args.seq_len, tokenizer.pad_token_id)

        with torch.no_grad():
            out_A = model(toks_A, output_hidden_states=True)
            out_Ap = model(toks_Ap, output_hidden_states=True)

        hidden_A = out_A.hidden_states[args.layer_idx]  # (1, L, d_model)
        hidden_Ap = out_Ap.hidden_states[args.layer_idx]

        token_pos = 0  # keep it simple for MVP
