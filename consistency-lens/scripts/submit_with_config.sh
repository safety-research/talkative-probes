...
submit_pretokenize_job() {
    ...
    if [ "$USE_SLURM" = true ]; then
        ...
        local result=$(sbatch --parsable \
            ...
            --wrap="bash -c 'cd $CONSISTENCY_LENS_DIR && source .venv/bin/activate && export OMP_NUM_THREADS=1 && export TOKENIZERS_PARALLELISM=false && export SLURM_CPU_BIND=none && export SLURM_CPUS_ON_NODE=$num_proc && python scripts/pretokenize_dataset.py --config-path=$CONSISTENCY_LENS_DIR/$(dirname $config) --config-name=$(basename $config .yaml)'" 2>&1)
