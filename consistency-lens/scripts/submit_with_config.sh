...
submit_pretokenize_job() {
    ...
    if [ "$USE_SLURM" = true ]; then
        ...
        local result=$(sbatch --parsable \
            ...
            --wrap="bash -c 'cd $CONSISTENCY_LENS_DIR && source .venv/bin/activate && export OMP_NUM_THREADS=$num_proc && export TOKENIZERS_PARALLELISM=true && python scripts/pretokenize_dataset.py --config-path=$CONSISTENCY_LENS_DIR/$(dirname $config) --config-name=$(basename $config .yaml)'" 2>&1)
