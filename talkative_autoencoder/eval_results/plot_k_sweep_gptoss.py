...
Usage:
    # Plot all gptoss JSON files in eval_results/
...
The script will:
- Find all gptoss*.json files if no files are specified
...
def _plot_combined_mse_histograms(all_data, output_file, bins=50):
    ...
    for result_file, data in all_data:
        results = data["results_by_k"]
        label_prefix = Path(result_file).stem.replace("gptoss", "").replace("_", " ").strip()
        for r in results:
            if "mse_values" not in r:
                continue
            mses = np.asarray(r["mse_values"])
...
def plot_variance_recovery(results_files, output_file="variance_recovery_plot.png", plot_histograms=False):
    ...
    for idx, (result_file, data) in enumerate(all_data):
        ...
        label = Path(result_file).stem.replace("gptoss", "").replace("_", " ").strip()
        if not label:
            label = f"Run {idx + 1}"
    ...
    plt.title("gptoss 9B Test Time Interpretability", fontsize=16, pad=20)
...
def plot_combined_mse_by_role(all_data, output_dir):
    ...
    for file_idx, (result_file, data) in enumerate(all_data):
        ...
        # Extract label from filename
        label_prefix = (
            Path(result_file).stem.replace("gptoss", "").replace("k_sweep_results_", "").replace("_", " ").strip()
        )
        if not label_prefix:
    ...
    output_path = output_dir / "gptoss_combined_mse_by_role.png"
    plt.savefig(output_path, dpi=300, bbox_inches="tight")
    plt.close()
...
def plot_all_json_files():
    """Find all gptoss JSON files and create a combined plot."""
    eval_dir = Path(__file__).parent

    # Find all gptoss k_sweep files
    json_files = sorted(eval_dir.glob("gptoss*.json"))
    json_files += sorted(eval_dir.glob("k_sweep_results*.json"))
    if not json_files:
        print("No gptoss*.json files found in eval_results/")
        return
    ...
    # Create plot
    plot_variance_recovery(json_files, output_file="gptoss_variance_recovery.png", plot_histograms=True)
...
def main():
    ...
    parser.add_argument("files", nargs="*", help="JSON result files to plot (if none, plots all gptoss files)")
    parser.add_argument(
        "-o",
        "--output",
