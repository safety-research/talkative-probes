...
def _plot_per_file_mse_histograms(result_file, data, output_dir, bins=50):
    ...
    plt.xlabel("MSE", fontsize=14)
    plt.ylabel("Count", fontsize=14)
    plt.title(f"MSE Distributions", fontsize=16)
    plt.grid(True, alpha=0.3)
    if len(results) > 1:
        plt.legend(loc="upper right", fontsize=10)
...
def plot_mse_by_role(result_file, data, output_dir):
    ...
    plt.title(f'MSE by Token Role', fontsize=16)
...
def plot_variance_recovery_by_role(result_file, data, output_dir):
    ...
    plt.title(f'Variance Recovery by Token Role', fontsize=16)
    plt.xticks(k_values, [str(k) for k in k_values])
    plt.grid(True, alpha=0.3)
    ...
    if plot_histograms:
        ...
        for result_file, data in all_data:
            ...
            if data["results_by_k"] and "activation_norms" in data["results_by_k"][0]:
                ...
                plt.ylabel("Count")
                plt.yscale("log")
                plt.title(f"Activation Vector Norms", fontsize=16)
...
def plot_combined_mse_by_role(all_data, output_dir):
    ...
    for file_idx, (result_file, data) in enumerate(all_data):
        ...
        # Extract label from filename
        label_prefix = Path(result_file).stem.replace("gemma3_k_sweep_full", "").replace("k_sweep_results_", "").replace("_", " ").strip()
        if not label_prefix:
        ...
        if user_data:
            ...
            plt.plot(k_vals, mse_vals, marker=marker_style, linestyle=line_style, 
                    color='#0072B2', linewidth=2.5, markersize=8, 
                    label=f'{label_prefix} - User', alpha=0.8)
            plot_has_data = True
        ...
        if assistant_data:
            ...
            plt.plot(k_vals, mse_vals, marker=marker_style, linestyle=line_style,
                    color='#D55E00', linewidth=2.5, markersize=8, 
                    label=f'{label_prefix} - Assistant', alpha=0.8)
            plot_has_data = True
    ...
    unique_k = sorted(set(k for _, data in all_data 
                         for r in data.get("results_by_k", []) 
                         if "mse_by_role" in r 
                         for k in [r["k"]]))
    if unique_k:
    ...
    output_path = output_dir / "combined_mse_by_role.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
...
def plot_combined_variance_recovery_by_role(all_data, output_dir):
    ...
    for file_idx, (result_file, data) in enumerate(all_data):
        ...
        if user_data:
            ...
            plt.plot(k_vals, vr_vals, marker=marker_style, linestyle=line_style, 
                    color='#0072B2', linewidth=2.5, markersize=8, 
                    label=f'{label_prefix} - User', alpha=0.8)
            plot_has_data = True
