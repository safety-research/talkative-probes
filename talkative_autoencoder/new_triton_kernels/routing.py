...
class SortTokens(torch.autograd.Function):
    ...
    def forward(ctx, expt_scal, expt_indx, n_expts_tot, bitmatrix):
        ...
        # scratchpad
        expt_offs = torch.zeros(n_expts_tot, dtype=torch.int32, device=device)
        combined_indx = torch.zeros(n_gates_pad * 2, dtype=torch.int32, device=device)
        # output
        topk_indx = combined_indx[:n_gates_pad]
        gate_indx = combined_indx[n_gates_pad:]
        gate_scal = torch.zeros(n_gates_pad, dtype=dtype, device=device)
...
def _compute_expt_data_internal(expt_hist, n_expts_tot, n_gates):
    ...
    token_offs_combined = torch.zeros((block_m_num + 1, pad(n_expts_tot + 1)), dtype=dtype, device=device)

    token_offs_raw = token_offs_combined[0][:n_expts_tot + 1]
    token_offs_pad = token_offs_combined[1:]

    block_pid_map = torch.zeros((block_m_num, pad(max_n_tiles)), dtype=dtype, device=device)
