...
def _get_kernel_cache_key(fused_activation_name, epilogue_name, device=None):
    """Create device-aware cache key"""
    if device is not None:
        # Use the actual device of the tensor being processed
...
def get_kernels(epilogue: FnSpecs = FnSpecs.default(), fused_activation: FnSpecs = FnSpecs.default(), device=None):
    global _kernels
    key = _get_kernel_cache_key(fused_activation.name, epilogue.name, device)
    
    # Thread-safe kernel creation with double-check pattern
    if key in _kernels:
        return _kernels[key]
    ...
    with _kernel_init_lock:
        ...
        # SAFER MODULE REGISTRATION: Use device-specific name to avoid conflicts
        # Extract device_id from the cache key (first element of the tuple)
        device_id = key[0]
        base_name = f"matmul_ogs_{fused_activation.name}_{epilogue.name}"
        module_name = f"{base_name}_device_{device_id}_{hash(key) & 0xFFFF}"
...
def apply_postprocessing_features(scatter_indx, finalize_scatter_idxs, opt_flags, expt_offs, num_indx, precision_config, routing_data,
    ...
    if postprocess_features.finalize:
        ...
        def compute_grid(BLOCK_N, num_warps):
            # Pass device from output tensor to get correct SM count
            device = out.device if hasattr(out, 'device') else None
            num_pid = target_info.num_sms(device=device) * (warps_per_sm // num_warps)
        ...
        # Pass the device from the output tensor to ensure correct kernel selection
        device = out_scatter.device if hasattr(out_scatter, 'device') else None
        kernels = get_kernels(epilogue.specs, fused_activation.specs, device=device)
...
def matmul_ogs(x, w, bias,
    ...
    # Get device capability for the specific device where x tensor is located
    if torch.cuda.is_available() and hasattr(x, 'device'):
        device_id = x.device.index if x.device.type == 'cuda' else 0
        device_capability = torch.cuda.get_device_capability(device_id)
    else:
        device_capability = (0, 0)
    can_use_tma = can_use_tma and (device_capability[0] > 9 or bitwidth(w.dtype) != 4)
    ...
    opt_flags = make_opt_flags(out_dtype, x.dtype, w.dtype, precision_config,
        ...
        device=x.device,  # Pass the device to ensure correct optimization flags
    )
    ...
    # Pass device from x to get correct SM count
    grid = min(target_info.num_sms(device=x.device) - opt_flags.idle_sms, max_grid) if opt_flags.is_persistent else max_grid
...
def matmul_ogs_torch(x, w, bias,
    ...
    for i, (lo, hi) in enumerate(offs):
        ...
        out = torch.matmul(round_x(x[batch, idx, :], torch.arange(lo, hi, device=x.device)).float(),
                           w[i].float())
        if bias is not None:
