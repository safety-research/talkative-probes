...
def downcast_to_mxfp(src_tensor: torch.Tensor, out_quant_type: torch.dtype, axis: int,
    ...
    out_quant_tensor = src_tensor.new_zeros(out_shape, dtype=out_quant_type)
    out_scale = src_tensor.new_zeros(out_scale_shape, dtype=torch.uint8)
...
def upcast_from_mxfp(tensor: torch.Tensor, scale: torch.Tensor, dtype: torch.dtype, axis: int):
    ...
    out = torch.zeros((*tensor.shape[:-1], logical_quant_dim), dtype=dtype, device=tensor.device)
