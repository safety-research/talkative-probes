...
def _sum_bitmatrix_rows(B, shape_bm, stride_bm: tl.constexpr, stride_bn: tl.constexpr,  # input bitmatrix
    ...
    tl.atomic_add(Ret + offs_n, tl.sum(ret, 0), sem="acq_rel")
...
def clear_sums(n_cols, device, MEMSET_BLOCK=512):
    ...
    out_ret = torch.zeros((blocks * MEMSET_BLOCK, ), device=device, dtype=torch.int32)
    _sum_bitmatrix_memset[(blocks, )](out_ret, MEMSET_BLOCK)
    # Ensure memset completes before returning
...
def sum_bitmatrix_rows(x, out_ret, partials_block_size=None):
    ...
    # Ensure atomic operations complete before returning
    torch.cuda.synchronize(device=out_ret.device)
