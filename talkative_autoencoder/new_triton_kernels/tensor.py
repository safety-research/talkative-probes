...
class Storage:
    ...
    def make_tma(self, block_shape, transpose=False):
        ...
        block_shape = list(block_shape)  # Make a copy to avoid modifying the original
        
        # Extend block_shape to match tensor dimensions if necessary
        # For missing leading dimensions, use the full size of that dimension
        if len(block_shape) < len(shape):
            missing_dims = len(shape) - len(block_shape)
            block_shape = shape[:missing_dims] + block_shape
        elif len(block_shape) > len(shape):
            # This shouldn't happen, but handle gracefully
            block_shape = block_shape[-len(shape):]
            
        # TODO
        # there is an issue w/ column-major TMA; we transpose instead
        transpose = self.data.stride()[-1] != 1
        if transpose:
            # Ensure we have at least 2 dimensions before transposing
            if len(block_shape) >= 2 and len(shape) >= 2 and len(strides) >= 2:
        ...
        if self.data.dtype == torch.uint8 and self.layout.name is None:
            ...
            if 1 in strides:
                ...
                # Ensure indx is within bounds of block_shape after potential transpose
                if indx < len(block_shape) and block_shape[indx] >= 2:
