...
def make_default_opt_flags_amd(
    ...
    if constraints.get("split_k", None) is not None:
        ...
        n_cu = torch.cuda.get_device_properties(device_id).multi_processor_count
...
def make_default_opt_flags_nvidia(
    ...
    if constraints.get("block_m", None):
        ...
        min_block_m = 64 if device_capability[0] == 10 else 16
    ...
    # Use the device_key which now represents the correct device
    device_id = device_key if isinstance(device_key, int) else (torch.cuda.current_device() if torch.cuda.is_available() else 0)
    ...
    if constraints.get("split_k", None) is not None:
        ...
        split_k = opt_flags_nvidia.compute_split_k(block_k, k, estimated_actual_grid_size, device_id=device_id)
    if split_k > 1:
    ...
    for ep in subtiles_to_check:
        ns = opt_flags_nvidia.compute_num_stages(*compute_num_stages_args, ep, epilogue_effective_itemsize, device_id=device_id)
        if ns > num_stages:
...
_opt_flags_lock = threading.Lock()
...
def reset_opt_flags():
    """Reset optimization flags for current device"""
    global _opt_flags
    with _opt_flags_lock:
        device_key = _get_device_key()
        if device_key in _opt_flags:
            del _opt_flags[device_key]
...
def make_opt_flags(
    ...
    # If device is provided, use it to get the correct device key
    ...
    if device_constraints.get("is_persistent", False) and not can_use_persistent_tma:
    ...
    # Check if device-specific flags are already set
    device_opt_flags = _opt_flags.get(device_key)
    if device_opt_flags is not None:
        assert not device_constraints
        return device_opt_flags
