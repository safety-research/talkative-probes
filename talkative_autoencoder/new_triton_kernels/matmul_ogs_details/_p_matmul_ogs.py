...
def _p_matmul_ogs(
    ...
    if YActualScale is not None:
        tl.atomic_max(YActualScale, compute_scale(local_absmax.to(tl.float32, bitcast=True), Y), sem="acq_rel")
    ...
    if device not in _per_device_alloc_fns:
        ...
        def alloc_fn(size: int, alignment: int, stream):
            ...
            if stream not in _per_stream_tensors or _per_stream_tensors[stream].numel() < size:
                _per_stream_tensors[stream] = torch.zeros(size, device=device, dtype=torch.int8)
