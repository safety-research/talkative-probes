...
def topk_forward(x, k, apply_softmax=True, dim=1, return_bitmatrix=True, y_indx=None, n_rows=None):
    ...
    y_vals = torch.zeros((n_rows_max, k), dtype=x.dtype, device=dev)
    if y_indx is not None:
        use_provided_indx = True
    else:
        y_indx = torch.zeros((n_rows_max, k), dtype=torch.int16, device=dev)
    ...
    bitmatrix = torch.zeros((n_cols_words, cdiv(n_rows_max, 32) * 32), dtype=torch.uint32, device=dev)
