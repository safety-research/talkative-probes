...
class BestOfKEvaluator:
            ...
            with torch.no_grad(), torch.amp.autocast("cuda", dtype=torch.bfloat16, enabled=self.use_bf16):
                ...
                if high_mse_mask.any() and debug_high_mse:
                    ...
                    for idx in high_mse_indices[:5]:  # Show first 5 high MSE cases
                        ...
                        print(
                            f"last 10 tokens: {escape_newlines(self.tokenizer.batch_decode(input_ids[start_idx + pos_in_batch : start_idx + pos_in_batch + 1, max(0, actual_position - 100) : min(actual_position + 1, input_ids.shape[1])], skip_special_tokens=False)[0])} on token '{current_token_text}' (id: {current_token_id})"
                        )
    ...
    def evaluate_with_k(
        ...
        cache_path = None
        cached_data = None
        use_cached = False  # Initialize here to ensure it's always defined
        if load_store and cache_dir and model_name and dataset_cfg:
            ...
            if dist.is_initialized() and dist.get_world_size() > 1:
                ...
                if cache_exists:
                    ...
                    cached_data = {
                        ...
                        "all_token_ids": cached_token_ids,
                    }
                else:
                    cached_data = None
            else:
            ...
            if (distributed and cache_exists and cached_data is not None) or (
                not distributed and cached_data is not None
            ):
                use_cached = True
                if distributed:
                ...
                if load_store:
                    ...
                    print("Extracting vectors without caching")
        ...
        if load_store and cache_path and not (load_store and cached_data is not None):
            ...
            if dist.is_initialized() and dist.get_world_size() > 1:
                ...
                if rank == 0:
                    ...
                    for i in range(world_size):
                        ...
                        if size > 0:
                            if i == 0:
                                ...
                                gathered_positions_list.append(
                                    torch.cat(all_positions_list, dim=0) if all_positions_list else torch.tensor([])
                                )
                                gathered_token_ids_list.append(
                                    torch.cat(all_token_ids_list, dim=0) if all_token_ids_list else torch.tensor([])
                                )
                            else:
                    ...
                    save_cached_vectors(
                        cache_path, all_A_gathered, all_positions_gathered, all_token_ids_gathered, metadata
                    )
                else:
                    # Other ranks send their data to rank 0
                    if all_A.shape[0] > 0:
                        all_positions_concat = (
                            torch.cat(all_positions_list, dim=0) if all_positions_list else torch.tensor([])
                        )
                        all_token_ids_concat = (
                            torch.cat(all_token_ids_list, dim=0) if all_token_ids_list else torch.tensor([])
                        )
...
def prepare_val_dataloader(
    ...
    val_sampler = FastDistributedSampler(
        val_dataset, num_replicas=world_size, rank=rank, shuffle=False, seed=cfg.get("seed", 42), drop_last=False
    )
...
def main(cfg: DictConfig) -> None:
    ...
    if "config" in ckpt:
        ...
        print("No config in checkpoint, just ensure eval config is accessible")
    ...
    if eval_cfg.get("load_store", False) and eval_cfg.get("num_positions", 1) == 1:
        ...
        if "encoder_config" in ckpt_temp:
            ...
            raise ValueError("Layer not found in checkpoint")
        ...
        print("No cache found or not loading from cache, will extract vectors in dataloader")
                ...
                if key == "k":
                    ...
                    if key in [
                        ...
                        gathered_results[key] = (
                            float(gathered_list[0].item()) if key != "total_positions" else int(gathered_list[0].item())
                        )
