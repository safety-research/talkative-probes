...
if [ ! -d "$PRETOKENIZED_PATH" ] || [ "$FORCE_REDUMP" == "true" ]; then
    ...
    python scripts/pretokenize_dataset.py \
        --config-path=conf --config-name=gpt2_pile_unfreeze
else
...
TRAIN_CMD="python scripts/01_train.py --config-name=gpt2_pile_unfreeze wandb.mode=online wandb.project=consistency-lens-gpt2-pile +wandb.name=\"gpt2_pile_unfreeze_${SLURM_JOB_ID}\" batch_size=256"
