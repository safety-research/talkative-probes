...
class InferenceService:
    ...
    def __init__(self, model_manager, settings: Settings, websocket_manager=None):
        ...
        for device in self.devices:
            for i in range(self.num_workers_per_gpu):
                ...
                self.workers[worker_name] = WorkerState(worker_id=worker_name, device=device)
                worker_id += 1
    ...
    async def start_processing(self):
        ...
        for worker_name, worker_state in self.workers.items():
            if worker_state.task is None or worker_state.task.done():
                worker_state.task = asyncio.create_task(self._worker_loop(worker_name))
        ...
        if len(self.devices) > 1:
            if (
                not hasattr(self, "load_balancer_task")
                or self.load_balancer_task is None
                or self.load_balancer_task.done()
            ):
    ...
    async def _dispatch_requests(self):
        ...
        while True:
            try:
                ...
                worker_name = await self._find_best_worker_for_request(request_id, text, options, request_type)
    ...
    async def _find_best_worker_for_request(
        self, request_id: str, text: str, options: dict[str, Any], request_type: str
    ) -> str:
        """
        ...
        if exclusive_gpu and exclusive_gpu in self.devices:
            ...
            workers_on_gpu = [(name, worker) for name, worker in self.workers.items() if worker.device == exclusive_gpu]
            if workers_on_gpu:
        ...
        if request_type == "group_switch":
            ...
            for name, worker in self.workers.items():
                ...
                if device_state:
                    ...
                    if device_state.current_group_id is None or device_state.current_group_id == target_group_id:
        ...
        if model_id:
            ...
            if candidate_devices:
                ...
                for device in candidate_devices:
                    workers_on_device = [
                        (name, worker) for name, worker in self.workers.items() if worker.device == device
                    ]
            ...
            best_device = self.model_manager.find_best_device_for_model(model_id, preferred_device=exclusive_gpu)

            # Find least busy worker on that device
            workers_on_device = [
                (name, worker) for name, worker in self.workers.items() if worker.device == best_device
            ]
        ...
        least_busy_worker = min(self.workers.items(), key=lambda x: x[1].queue.qsize())
        ...
        while True:
            try:
                ...
                try:
                    ...
                    await self._handle_single_request(request_id, text, options, request_type, worker.device)
    ...
    async def _handle_single_request(
        self, request_id: str, text: str, options: dict[str, Any], request_type: str, device: str
    ):
        ...
        try:
            ...
            if request_type == "group_switch":
                ...
                if (
                    target_group_id
                    and device_state
                    and (target_group_id != device_state.current_group_id or device_state.is_switching)
                    and not already_waiting
                ):
                    # Need to switch groups on this device
                    await self._handle_group_switch_for_request(
                        request_id, text, options, request_type, target_group_id, model_id, device
                    )
                    return
                ...
                if already_waiting:
                    ...
                    if (
                        device_state
                        and device_state.current_group_id == target_group_id
                        and not device_state.is_switching
                    ):
                        # Group is now loaded and ready, proceed with request
                        logger.info(
                            f"Group {target_group_id} now loaded on {device}, proceeding with request {request_id}"
                        )
                ...
                try:
                    analyzer, used_device = await self.model_manager.get_analyzer_for_model(model_id, device)
                ...
                if request and request["status"] != "cancelled":
                    ...
                    try:
                        if request_type == "generate":
                            await self._process_generation_request(request_id, text, options, analyzer, device)
                        elif request_type == "send_message":
                            await self._process_send_message_request(request_id, text, options, analyzer, device)
                        else:
                            await self._process_analysis_request(request_id, text, options, analyzer, device)
                    except Exception as e:
    ...
    async def _handle_group_switch_for_request(
        ...
        switch_already_queued = any(
            req.get("type") == "group_switch"
            and req.get("target_group_id") == target_group_id
            and req.get("device") == device
            and req.get("status") in ["queued", "processing"]
        ...
        if not switch_already_queued:
            ...
            switch_request_id = await self.queue.add_group_switch_request(target_group_id, model_id, websocket)
    ...
    async def _process_analysis_request(
        self, request_id: str, text: str, options: dict[str, Any], analyzer: Any, device: str
    ):
        ...
        try:
            ...
            if options.get("use_chat_format", False) and not used_prior_generated_text:
                if isinstance(text, str):
                    ...
                    text_to_analyze = await self._parse_chat_format(
                        text, model_id, analyzer.tokenizer if hasattr(analyzer, "tokenizer") else None
                    )
                    ...
                    if chat_tokenizer and hasattr(chat_tokenizer, "apply_chat_template"):
                        ...
                        if "gemma" in model_id and messages and messages[0].get("role") == "system":
                            ...
                            if messages:
                                ...
                                logger.info("Merged system prompt into first message")
                        ...
                        raise ValueError(
                            f"Chat format not supported for analysis due to lack of chat tokenizer for {model_id}"
                        )
            ...
            if batch_size is not None:
                ...
                logger.info(
                    f"Auto-calculated batch_size: {batch_size} (auto_batch_max={auto_batch_max}, k_rollouts={k_rollouts}) for request {request_id}"
                )
            ...
            logger.info(
                f"Starting analysis in executor for request {request_id} on {device} with batch_size={batch_size}, k_rollouts={k_rollouts}"
            )
        ...
        try:
            ...
            if is_chat:
                ...
                if chat_tokenizer:
                    ...
                    logger.info("Using chat tokenizer for generation")

            completions = await loop.run_in_executor(
                None, lambda: analyzer.generate_continuation(prompt, **generation_kwargs)
            )
            ...
            if websocket:
                await self.queue._send_websocket_update(
                    websocket, {"type": "generation_error", "request_id": request_id, "error": str(e), "device": device}
                )
    ...
    async def _process_send_message_request(
        ...
        try:
            ...
            if websocket:
                await self.queue._send_websocket_update(
                    websocket,
                    {"type": "processing", "request_id": request_id, "context": "send_message", "device": device},
                )
            ...
            if chat_tokenizer:
                ...
                logger.info("Using chat tokenizer for send_message")

            # Run send_message in executor
            loop = asyncio.get_event_loop()
            resp = await loop.run_in_executor(None, lambda: analyzer.send_message(messages, **send_message_kwargs))
            if analyzer.thinking_enabled:
            ...
            if websocket:
                await self.queue._send_websocket_update(
                    websocket,
                    {"type": "send_message_error", "request_id": request_id, "error": str(e), "device": device},
                )
    ...
    async def _parse_chat_format(self, text: str, model_id: str, tokenizer=None) -> str:
        ...
        try:
            ...
            if messages is None:
                logger.info("Converting plain text to chat format for analysis")
                messages = [{"role": "user", "content": text}]
            ...
            if chat_tokenizer and hasattr(chat_tokenizer, "apply_chat_template"):
                if "gemma" in model_id and messages[0]["role"] == "system":
                    ...
                    logger.info("Added system prompt for Gemma as user prefix")
    ...
    def get_device_load_stats(self) -> dict[str, dict[str, Any]]:
        ...
        for device in self.devices:
            ...
            device_workers = [w for w in self.workers.values() if w.device == device]
                ...
                for device, stats in device_stats.items():
                    ...
                    if stats["queue_depth"] > 3 or stats["utilization"] > 0.7:
                        ...
                        if device_state and device_state.current_group_id:
                            ...
                            logger.debug(
                                f"Device {device} is busy (queue={stats['queue_depth']}, util={stats['utilization']:.2%}), considering duplication of group {group_id}"
                            )

                            new_device = await self.model_manager.consider_duplicating_group(group_id, device_stats)

                            if new_device:
                                logger.info(
                                    f"Successfully duplicated group {group_id} to {new_device} for load balancing"
                                )
