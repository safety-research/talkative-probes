...
class InferenceQueue:
    def __init__(self, max_size: int = 100):
        ...
        self.websocket_manager = None  # Will be set by ModelManager
        self._processing_count = 0  # Track actively processing requests
    ...
    def get_queue_stats(self) -> dict[str, int]:
        ...
        queued_count = sum(1 for req in self.active_requests.values() if req["status"] == "queued")
        processing_count = sum(1 for req in self.active_requests.values() if req["status"] == "processing")
    ...
    def get_queued_request_ids(self) -> list[str]:
        """Get ordered list of queued request IDs"""
        # Get all queued requests sorted by created_at time
        queued_requests = [
            (req_id, req["created_at"]) for req_id, req in self.active_requests.items() if req["status"] == "queued"
        ]
        # Sort by creation time to maintain FIFO order
        queued_requests.sort(key=lambda x: x[1])
        return [req_id for req_id, _ in queued_requests]

    def get_position_in_queue(self, request_id: str) -> int | None:
        """Get the position of a request in the queue (1-indexed)"""
        queued_ids = self.get_queued_request_ids()
        if request_id in queued_ids:
            return queued_ids.index(request_id) + 1
        return None

    def cleanup_stale_requests(self, max_age_minutes: int = 30):
        """Remove stale requests that have been completed/failed for too long"""
        current_time = datetime.utcnow()
        stale_requests = []

        for req_id, req in self.active_requests.items():
            # Only clean up completed, failed, or cancelled requests
            if req["status"] in ["completed", "failed", "cancelled"]:
                if "completed_at" in req:
                    age = (current_time - req["completed_at"]).total_seconds() / 60
    ...
    async def _broadcast_queue_update(self):
        ...
        if self.websocket_manager:
            # Cleanup stale requests before broadcasting
            ...
            await self.websocket_manager.broadcast(
                {
                    ...
                    "queued_ids": queued_ids,  # Include ordered list of queued request IDs
                }
            )


class ModelManager:
    def __init__(self, checkpoint_path: str, max_retries: int = 3, websocket_manager=None):
        ...
        self.queue.websocket_manager = websocket_manager  # Set the websocket manager
                    ...
                    try:
                        test_result = self.analyzer.analyze_all_tokens(
                            ...
                            optimize_explanations_config={
                                "best_of_k": 1,  # Minimal rollouts for test
                                "use_batched": True,
                            },
                        )
    ...
    async def _process_queue(self):
        ...
        while True:
            try:
                ...
                # Broadcast queue update
                await self.queue._broadcast_queue_update()

                # Check if client is still connected (for WebSocket)
                if (
                    hasattr(request, "websocket")
                    and request.get("websocket")
                    and hasattr(request["websocket"], "client_state")
                    and request["websocket"].client_state != 1
                ):
                ...
                try:
                    ...
                    async with self._load_lock:
                        if self.analyzer is None:
                            ...
                            if "websocket" in request and request["websocket"]:
                                await request["websocket"].send_json(
                                    {"type": "status", "message": "Model loaded successfully!"}
                                )
    ...
    async def _process_generation_request(self, request: dict, request_id: str):
        ...
        try:
            ...
            if "websocket" in request and request["websocket"]:
                await request["websocket"].send_json(
                    {"type": "processing", "message": "Generating text...", "context": "generation"}
                )
            ...
            if options.get("is_chat", False):
                try:
                    ...
                    continuations = await loop.run_in_executor(
                        ...
                        lambda: self.analyzer.generate_continuation(messages, **generation_kwargs),
                    )
                except Exception as e:
            ...
            request["result"] = {"continuations": continuations}
        ...
        if "websocket" in request and request["websocket"]:
            try:
                if request["status"] == "cancelled":
                    await request["websocket"].send_json(
                        {"type": "interrupted", "request_id": request_id, "context": "generation"}
                    )
                else:
        ...
        # Broadcast queue update after request completes
        await self.queue._broadcast_queue_update()
    ...
    async def _process_analysis_request(self, request: dict, request_id: str):
        ...
        try:
            ...
            if "websocket" in request and request["websocket"]:
                await request["websocket"].send_json(
                    {"type": "processing", "message": "Starting text analysis...", "context": "analysis"}
                )
        ...
        if "websocket" in request and request["websocket"]:
            try:
                if request["status"] == "cancelled":
                    await request["websocket"].send_json(
                        {"type": "interrupted", "request_id": request_id, "context": "analysis"}
                    )
                else:
                    await request["websocket"].send_json(
                        {
