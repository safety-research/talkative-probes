...
def print_verbose_sample_details(
    ...
    if sample_losses is not None:
        ...
        print(
            f"  - MSE Loss (A_i vs A_hat): {sample_losses['mse']:.4f} (weighted: {sample_losses['mse_weighted']:.4f})"
        )
        print(
        ...
        print(
            "  - KL Loss"
            + (" (A_hat+Δ)" if resample_ablation else "(A_hat)")
            + f": {sample_losses['kl']:.4f} (weighted: {sample_losses['kl_weighted']:.4f})"
        )
    ...
    if decoder_tokens:
        ...
        dec_data_rows = [
            decoder_tokens
            + ([f"[{original_token_at_p_str}]" if add_current_token else ""] if add_current_token else [])
...
def compute_single_sample_losses(
    ...
    loss_dict = {
        ...
        "mse_weighted": _loss_fns["mse_weight"]
        * (losses["mse"].item() if torch.is_tensor(losses["mse"]) else losses["mse"]),
        "lm_weighted": (_loss_fns["lm_base_weight"] * _loss_fns["alpha"])
        * (losses["lm"].item() if torch.is_tensor(losses["lm"]) else losses["lm"]),
        "kl_weighted": _loss_fns["kl_base_weight"]
        * (losses["kl"].item() if torch.is_tensor(losses["kl"]) else losses["kl"]),
        "entropy_weighted": -_loss_fns["entropy_weight"]
        * (losses["entropy"].item() if torch.is_tensor(losses["entropy"]) else losses["entropy"]),
...
def process_and_print_verbose_batch_samples(
    ...
    with torch.no_grad():
        ...
        for i in sample_indices:
            ...
            models_with_orig = {"dec": dec, "enc": enc, "orig": orig}
            ...
            logits_train_at_p_batched = (
                computed_tensors["logits_A_train_kl_pos"].detach() if do_kl_computation else None
            )
            ...
            A_hat_baseline = (
                enc(
                    ...
                    current_token_ids=batch["input_ids_A"][i][p].to(device).unsqueeze(0)
            ...
            if cfg.get("decoder_prompt"):
                ...
                A_hat_hard_prompt = (
                    enc(
                        base_gen_single_hard_for_enc.generated_text_embeddings,
                        current_token_ids=batch["input_ids_A"][i][p].to(device).unsqueeze(0)
                        if enc.config.add_current_token
                        else None,
                    )
            ...
            if do_orig_model_computations:
                ...
                top_n_lens_recon_tokens = ["N/A"] * top_n_analysis
            ...
            if comparison_tuned_lens is not None:
                if (
                    hasattr(comparison_tuned_lens, "config")
                    and hasattr(comparison_tuned_lens.config, "num_hidden_layers")
                    and l < comparison_tuned_lens.config.num_hidden_layers
                ):
                    try:
                        with torch.no_grad():
                            A_i_casted = A_i.to(torch.float32)
                            logits_tuned_lens_at_p_batched = comparison_tuned_lens(A_i_casted, idx=l).detach().clone()
                        top_n_tuned_lens_tokens = get_top_n_tokens(
                            logits_tuned_lens_at_p_batched.squeeze(0), tok, top_n_analysis
                        )
                        del A_i_casted
            ...
            if (
                A_prime_i is not None
                and "input_ids_A_prime" in batch
                and "token_pos_A_prime" in batch
                and idx_for_aprime < batch["input_ids_A_prime"].size(0)
                and idx_for_aprime < batch["token_pos_A_prime"].size(0)
            ):
                ...
                before_p_prime = (
                    tok.decode(raw_prime_ids[crop_start_idx_prime:p_prime]) if p_prime > crop_start_idx_prime else ""
                )
                token_at_p_prime = tok.decode([raw_prime_ids[p_prime]]) if p_prime < len(raw_prime_ids) else ""
                after_p_prime = (
                    tok.decode(raw_prime_ids[p_prime + 1 : crop_end_idx_prime])
                    if p_prime + 1 < crop_end_idx_prime
                    else ""
                )
            ...
            if crop_end_idx < len(raw_prefix_ids):
                original_string_cropped = original_string_cropped + "..."
            original_token_at_p_str = (
                escape_newlines(tok.decode([raw_prefix_ids[p]])) if p < len(raw_prefix_ids) else "N/A"
            )
            ...
            def safe_kl(l1, l2):
                if not do_kl_computation or l1 is None or l2 is None:
                    return None
            ...
            kl_divergences_for_print_section = {
                ...
                "From Original Activation A (training objective)": {
                    ...
                    (
                        "KL(A || A_hat+Δ) [TRAINING LOSS]" if resample_ablation else "KL(A || A_hat) [TRAINING LOSS]"
                    ): training_kl_loss_value,
            ...
            if return_structured_data:
                ...
                if logits_orig_at_p_batched is not None:
                    ...
                    orig_top_values, orig_top_indices = torch.topk(
                        orig_probs, k=min(top_n_analysis, orig_probs.size(-1))
                    )
                ...
                if logits_approx_at_p_batched is not None:  # Logits from A_hat intervention
                    ...
                    recon_top_values, recon_top_indices = torch.topk(
                        recon_probs, k=min(top_n_analysis, recon_probs.size(-1))
                    )
                ...
                if (
                    gen_single.raw_lm_logits.numel() > 0 and gen_single.raw_lm_logits.size(1) > 0
                ):  # Check if any logits generated
                ...
                sample_data = {
                    ...
                    "continuation": escape_newlines(autoregressive_continuation)
                    if autoregressive_continuation
                    else None,
            ...
            if capture_output:
                ...
                with redirect_stdout(output_buffer):
                    print_verbose_sample_details(
                        ...
                        kl_divergences=kl_divergences_for_print_section,
                        resample_ablation=resample_ablation,
                    )
...
def get_soft_prompt_projections(
    ...
    if dec.prompt_left_emb is not None or dec.prompt_right_emb is not None:
        ...
        if dec.prompt_left_emb is not None:
            ...
            left_input_proj = project_embeddings_to_text(dec.prompt_left_emb, input_emb_table, tok, top_k=3)
            decoder_projections["left_input"] = left_input_proj

            # Using output embeddings
            left_output_proj = project_embeddings_to_text(dec.prompt_left_emb, output_emb_table, tok, top_k=3)
            decoder_projections["left_output"] = left_output_proj

        # Project right prompt
        if dec.prompt_right_emb is not None:
            # Using input embeddings
            right_input_proj = project_embeddings_to_text(dec.prompt_right_emb, input_emb_table, tok, top_k=3)
            decoder_projections["right_input"] = right_input_proj

            # Using output embeddings
            right_output_proj = project_embeddings_to_text(dec.prompt_right_emb, output_emb_table, tok, top_k=3)
    ...
    if enc.soft_prompt_embeddings is not None:
        ...
        encoder_proj = project_embeddings_to_text(enc.soft_prompt_embeddings, input_emb_table, tok, top_k=3)
        ...
        projections["encoder"] = {"prefix": encoder_proj, "postfix": encoder_postfix_proj}
...
def build_topk_preds_by_rank(raw_lm_logits, num_tokens, k, tok):
    ...
    for logit_slice in raw_lm_logits[0][:num_tokens]:
        ...
        for rank_idx, tok_id in enumerate(topk_ids):
            preds_by_rank[rank_idx].append(escape_newlines(tok.decode([tok_id]).strip()))
