...
class Generated(NamedTuple):
    ...
    def detach(self):
        return Generated(
            ...
            if hasattr(self.generated_text_embeddings, "detach")
    ...
    def cpu(self):
        return Generated(
            ...
            if hasattr(self.generated_text_embeddings, "cpu")
            else self.generated_text_embeddings,
    ...
    def clone(self):
        return Generated(
            ...
            if hasattr(self.generated_text_embeddings, "clone")
            else self.generated_text_embeddings,
...
class Decoder(nn.Module):
    ...
    def __init__(self, cfg: DecoderConfig, base_to_use: Optional[PreTrainedModel] = None) -> None:
        ...
        n_layers_base = (
            ...
            if not hasattr(self.base.config, "text_config")
            else self.base.config.text_config.num_hidden_layers
        )
        self.config.final_logit_softcapping = None  # self.base.config.final_logit_softcapping if hasattr(self.base.config, "final_logit_softcapping") else None
        ...
        if self.config.subtract_add_pos_embeddings:
            ...
            num_pos_embeddings = (
                self.base.config.max_position_embeddings
                if not hasattr(self.base.config, "text_config")
                else self.base.config.text_config.max_position_embeddings
            )
    ...
    def swap_base_model(self, model_name_or_path: str, keep_projection: bool = True) -> None:
        ...
        self.base = AutoModelForCausalLM.from_pretrained(
            model_name_or_path, attn_implementation=self.config.attn_implementation
        )
        ...
        vocab_size = (
            ...
            if not hasattr(self.base.config, "text_config")
            else self.base.config.text_config.vocab_size
        )
        n_layers_base = (
            self.base.config.num_hidden_layers
            if not hasattr(self.base.config, "text_config")
        ...
        if self.config.per_layer_projections:
            ...
            if (
                keep_projection
                and self.proj_weight is not None
                and old_proj_weight.shape == (n_proj_layers, d_model, d_model)
            ):
    ...
    def _patched_forward(
        ...
        if hasattr(main_base, "transformer") and not self.is_gemma:  # Check it's not Gemma2 disguised as GPT-2
            # --- GPT-2 Style ---
            ...
            if (
                gpt2_cache_position_for_mask is None
            ):  # Should align with how position_ids was set if cache_position was None
                past_seen = past_key_values.get_seq_length() if past_key_values is not None else 0
                gpt2_cache_position_for_mask = torch.arange(
                    past_seen, past_seen + seq_length, device=device, dtype=torch.long
                )
            ...
            if use_cache and past_key_values is None:
                ...
                ):  # Create DynamicCache if nothing is passed and HybridCache not made by caller
                    past_key_values = DynamicCache()
    ...
    def generate_soft(
        ...
        * Returns the *text* part only (excludes the A_proj token) so caller
        ...
        if do_patching:
            ...
            parts.append(
                main_base.get_input_embeddings()
                .weight[special_token.squeeze().item() if isinstance(special_token, torch.Tensor) else special_token]
                .clone()
                .unsqueeze(0)
                .repeat(B, 1, 1)
            )
    ...
    def check_for_nans(self) -> bool:
        ...
        try:
            ...
            if torch.isnan(input_emb_weight).any():
                log.error("NaN detected in base model input embeddings")
                nan_found = True
    ...
    def generate_soft_kv_cached(
        ...
        if self.config.patch_all_layers:
            ...
            if self.is_gemma and HybridCache is not None:
                ...
                past_key_values = HybridCache(
                    ...
                    if not hasattr(main_base.config, "text_config")
                    else main_base.config.text_config,
                    max_batch_size=B,
        ...
        if not self.config.end_to_end:
            ...
            if self.is_gemma3:
                copy_of_last_of_last_hidden_state = copy_of_last_of_last_hidden_state * self.normalizer
        ...
        if not self.config.end_to_end:
            ...
            if self.is_gemma3:  # apparently copy_last-Hidden already has multiplied normaliser
                newembs = newembs * self.normalizer
            ...
            if self.is_gemma3 or self.is_gptoss:
                ...
                final_cache_position = torch.arange(
                    current_position - max_length, current_position, device=device, dtype=torch.long
                )
                outputs = main_base(
                    inputs_embeds=inputs_embeds,
    ...
    def generate_soft_kv_cached_nondiff(
        ...
        if add_tokens is not None:
            ...
            added_token_embeddings = input_emb_table[token_ids_tensor].unsqueeze(0).expand(B, -1, -1)
            parts.append(added_token_embeddings)
            print(f"new length, (added {len(add_tokens)} tokens)")
        seq_embs = torch.cat(parts, dim=1)
